...
import PspTaxDetailSheet from "./PspTaxDetailSheet";
...
const PspTaxReportsTab = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      result = result.filter((r) => r.pspName.toLowerCase().includes(q));
    }
        ...
        <table className="w-full min-w-[1000px] border-collapse text-left">
          <thead>
            <tr>
              ...
              <th className={thClass}>Taxable Amount</th>
                ...
                <tr key={row.id} className="hover:bg-[#F9FAFB]/60 transition-colors">
                  <td className={tdClass}>
                    <div className="flex items-center gap-3">
                      {row.isBusiness ? (
                        <div className="flex size-8 shrink-0 items-center justify-center rounded-full bg-[#E0EAFF] text-xs font-bold text-[#175CD3]">
                          {row.pspName.charAt(0)}
                        </div>
                      ) : (
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              <select
                value={fPspType}
                onChange={(e) => setFPspType(e.target.value)}
                className="h-11 w-full appearance-none rounded-lg border border-[#D0D5DD] bg-white px-3.5 pr-10 text-sm text-[#344054] focus:border-[#F04438] focus:outline-none focus:ring-1 focus:ring-[#F04438] cursor-pointer shadow-[0_1px_2px_0_rgba(16,24,40,0.05)]"
              >
...
export default PspTaxReportsTab;
