...
const FieldValue = ({ children, className = "" }) => (
  <p className={`text-sm font-medium text-[#475467] ${className}`}>{children}</p>
);
...
const PlatformTaxDetailSheet = ({ isOpen, onClose, entry }) => {
  ...
  return (
    <SideSheet
      isOpen={isOpen}
      onClose={onClose}
      title={`Platform Tax Details • ${entry.period}`}
      widthClass="w-[480px]"
      footer={
      ...
      <div className="pb-4">
        ...
        <div className="grid grid-cols-2 gap-y-5 gap-x-4">
          <div>
            ...
            <FieldValue className="text-[#175CD3] font-medium">
              {entry.revenueType}
            </FieldValue>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="mt-1">
              <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${STATUS_STYLE[entry.status] || "text-[#667085]"}`}>
                {entry.status}
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-y-5 gap-x-4 mb-4">
          <div>
            ...
            <FieldValue>{entry.amount}</FieldValue>
          </div>
          <div>
            <FieldLabel>Final Tax</FieldLabel>
            <FieldValue className="text-[#101828] font-bold">{entry.tax}</FieldValue>
          </div>
        </div>
        <p className="text-sm text-[#475467] leading-relaxed">
          Platform tax is automatically allocated from base platform revenue elements.
        </p>
...
export default PlatformTaxDetailSheet;
