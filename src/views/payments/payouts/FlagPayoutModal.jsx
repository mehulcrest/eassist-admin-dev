...
const FlagPayoutModal = ({ isOpen, onClose, payout }) => {
  const [issueType,    setIssueType]    = useState("");
  const [description,  setDescription]  = useState("");
  const [showDropdown, setShowDropdown] = useState(false);

  const handleSubmit = () => {
    console.log("Payout issue flagged:", { payoutId: payout?.id, issueType, description });
    setIssueType("");
    setDescription("");
    onClose();
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-[460px] rounded-2xl bg-white shadow-xl p-6">
        ...
        {/* Title */}
        <h2 className="text-lg font-bold text-[#101828]">Flag Issue</h2>
        <p className="mt-1 text-sm text-[#667085]">
          Flag this payout transaction for suspicious or abnormal activity.
          <br />
...
export default FlagPayoutModal;
