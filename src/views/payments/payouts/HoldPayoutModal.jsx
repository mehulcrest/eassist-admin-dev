...
const HoldPayoutModal = ({ isOpen, onClose, payout }) => {
  const [reason,       setReason]       = useState("");
  const [notes,        setNotes]        = useState("");
  const [showDropdown, setShowDropdown] = useState(false);

  const handleSubmit = () => {
    console.log("Payout hold submitted:", {
      payoutId: payout?.id,
      party: payout?.party,
      reason,
      notes,
  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-[460px] rounded-2xl bg-white shadow-xl p-6">
        ...
        {/* Title */}
        <h2 className="text-lg font-bold text-[#101828]">Hold PSP Payout</h2>
...
export default HoldPayoutModal;
